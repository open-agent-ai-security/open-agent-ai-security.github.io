---
title: Praxen 2.0 Beta: Threat Modeling, Drawn from the Code Itself
author: Steve Wilson
date: 2026-09-08
published: yes
summary: The Praxen 2.0 public beta turns an agent's own code into a live, visual threat model — trust boundaries, STRIDE threats, and attack paths you can follow by hovering, with every element citing file:line evidence. Install it side by side with the stable release today.
tags: release, praxen
...
Today we're opening the **public beta of [Praxen](https://open-agent-ai-security.github.io/praxen/) 2.0** — the biggest release since the project launched. It installs side by side with the stable release, so you can try it without touching the Praxen you already run.
...
We didn't ship a feature and a promise. Every target in Praxen's public 12-agent benchmark ships with a hosted threat model today, linked beside its analysis report and remit on the beta [suite health page](https://open-agent-ai-security.github.io/praxen/beta/tests/baselines/suite-health-report.html). Browse them like a gallery of agent security architecture: [OpenHands](https://open-agent-ai-security.github.io/praxen/beta/tests/baselines/v1.3-opus5/openhands/openhands-threatmodel-2026-08-21-143615.html), the autonomous software engineer; [Hermes](https://open-agent-ai-security.github.io/praxen/beta/tests/baselines/v1.3-opus5/hermes-agent-desktop/hermes-agent-desktop-threatmodel-2026-08-21-143615.html), a desktop assistant reachable from two dozen chat platforms; [OpenAI's customer-service demo](https://open-agent-ai-security.github.io/praxen/beta/tests/baselines/v1.3-opus5/openai-customer-service/openai-customer-service-threatmodel-2026-08-21-143615.html), anonymous caller to booking write. Three very different agents; the same five lanes; very different attack stories.
...
The beta installs from the same community marketplace as the stable release, under its own name — `praxen-beta` — so the two live side by side. Your production `praxen` install is untouched, and stable 1.3 remains what `praxen@open-agent-ai-security` gives you until 2.0 goes final.
...
One tip if you already run stable Praxen: disable it while you're testing (`claude plugin disable praxen@open-agent-ai-security`) so there's no ambiguity about which version answers a scan request. Re-enable it when you're done.

Then point the beta at an agent and say *"run a Praxen analysis with a threat model."* The **beta documentation lives at [open-agent-ai-security.github.io/praxen/beta](https://open-agent-ai-security.github.io/praxen/beta/)** — start with the [installation guide](https://open-agent-ai-security.github.io/praxen/beta/guide/installation.html), then the [threat modeling guide](https://open-agent-ai-security.github.io/praxen/beta/guide/threat-modeling.html) for how to run and read a model, and the [methodology deep-dive](https://open-agent-ai-security.github.io/praxen/beta/guide/threat-modeling-methodology.html) for the STRIDE mapping, the boundary archetypes, and how attack paths are proven.

## Beta means we want your feedback

This is a real beta, not a soft launch: the threat modeler has passed its quality gates on our 12-agent benchmark, and now we want it on *your* agents — the weird ones, the big ones, the ones with an MCP server nobody remembers adding. We welcome bugs and suggestions via [GitHub issues](https://github.com/open-agent-ai-security/praxen/issues) — if a diagram comes out wrong, a citation doesn't hold up, the model misses a boundary you know is there, or you can see what the next version should do, that's exactly what we want to hear. When 2.0 goes final it will ship through the standard `praxen` install, and `praxen-beta` will retire.
