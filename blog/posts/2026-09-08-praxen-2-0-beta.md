...
## Beta means we want your bug reports

This is a real beta, not a soft launch: the threat modeler has passed its quality gates on our 12-agent benchmark, and now we want it on *your* agents — the weird ones, the big ones, the ones with an MCP server nobody remembers adding. If a diagram comes out wrong, a citation doesn't hold up, or the model misses a boundary you know is there, [file an issue](https://github.com/open-agent-ai-security/praxen/issues) — that's the whole point of the exercise. When 2.0 goes final it will ship through the standard `praxen` install, and `praxen-beta` will retire.
